import { Code, Cpu, Globe, CheckCircle2, Zap, MessageSquare } from 'lucide-react';
...
const AboutSection = () => {
  ...
  const values = [
    { name: 'Timeliness', icon: <Zap className="size-4" />, text: 'On-time delivery' },
    { name: 'Attention to Detail', icon: <CheckCircle2 className="size-4" />, text: 'Pixel perfect' },
    { name: 'Clear Communication', icon: <MessageSquare className="size-4" />, text: 'Transparent' },
  ];

  return (
    <section id="about" className="w-full py-24 bg-[#0a0a0a] text-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-4xl md:text-5xl font-bold text-center">
            ...
            <span className="text-gray-500 italic font-light text-3xl md:text-4xl">not just interfaces.</span>
          </h2>
        </motion.div>
          ...
          >
            <div className="space-y-4">
              <p className="text-xl text-gray-300 leading-relaxed">
                Hi, I'm <span className="text-white font-semibold italic underline decoration-[#4584f7]">Akmal Dzakwan Faiz</span>.
                I bridge the gap between <span className="text-white font-semibold">design and technology</span> by creating applications that are not only beautiful but also highly performant.
              </p>
              <p className="text-gray-400 leading-relaxed text-justify">
                Sejak memulai perjalanan saya sebagai developer, saya fokus membangun produk digital yang membantu bisnis berkembang. 
                Ketertarikan saya pada <span className="text-blue-400">Web Development</span> bermula dari rasa penasaran bagaimana logika bisa diubah menjadi interaksi visual yang bermakna. 
                Fokus utama saya adalah skalabilitas dan pengalaman pengguna yang mulus.
              </p>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              {values.map((v, i) => (
                <div key={i} className="flex items-center gap-2 bg-white/5 border border-white/10 px-3 py-1.5 rounded-full">
                  ...
                  <span className="text-[10px] md:text-xs font-medium uppercase tracking-wider text-gray-300">{v.name}</span>
                </div>
              ))}
            </div>
